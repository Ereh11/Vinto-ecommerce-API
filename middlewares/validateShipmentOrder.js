...
const validateShipmentOrder = async (req, res, next) => {
  ...
  if (!shipmentInfo)
    return sendResponse(
      ...
      400,
      { shipment: null },
      "shipmentInfo is required"
    );

  if (!validator.isMongoId(cart))
    return sendResponse(
      res,
      status.Fail,
      400,
      { shipment: null },
      "cart need to be a valid MongoId"
    );
  if (!validator.isMongoId(shipmentInfo))
    return sendResponse(
      res,
      status.Fail,
      400,
      { shipment: null },
      "shipmentInfo need to be a valid MongoId"
    );

  const cartExist = await mongoose.model("Cart").exists({ _id: cart });
  const shipmentInfoExist = await mongoose
    .model("ShipmentInfo")
    .exists({ _id: shipmentInfo });
  if (!cartExist)
    return sendResponse(
      res,
      status.Fail,
